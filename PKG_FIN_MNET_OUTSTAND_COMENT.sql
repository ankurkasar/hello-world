...
FUNCTION prc_fin_ousncom_liab_sum_new (
      i_lic_number           IN   fid_license.lic_number%TYPE,
      i_lic_currency         IN   fid_license.lic_currency%TYPE,
      i_param11              IN   DATE,
      i_lic_price            IN   fid_license.lic_price%TYPE,
...
FUNCTION prc_fin_ousncom_paid_sum_new (
      i_lic_number     IN   fid_license.lic_number%TYPE,
      i_lic_currency   IN   fid_license.lic_currency%TYPE,
      i_lsl_number     IN   fid_payment.pay_lsl_number%TYPE,
      i_param11        IN   DATE
...
**********************************************************************************************************************************************/
   ...
   BEGIN
      ...
      IF  i_period_date >= v_go_live_date
      THEN
            ...
            l_qry_string :=
                 ...
                 -- ,    round(fl.lic_price,2) lic_price
                    ...
                    ,CASE WHEN '''
                             ...
                             || l_year
                             || ')
                    ELSE PKG_FIN_MNET_OUTSTAND_COMENT.PRC_FIN_MNET_OUSNCOM_LIAB(fl.lic_number , con_number , fl.lic_currency , last_day('''
                             || i_period_date
                             || '''), lsl.lsl_lee_price , fl.lic_markup_percent,lsl.lsl_number)END
                    +
                    PKG_FIN_MNET_OUTSTAND_COMENT.PRC_FIN_MNET_OUSNCOM_PAID(fl.lic_number , con_number , fl.lic_currency ,lsl.lsl_number, last_day('''
                             || i_period_date
                             || ''')    ) fee
                    ,    ROUND(PKG_FIN_MNET_OUTSTAND_COMENT.PRC_FIN_MNET_OUSNCOM_PAID(fl.lic_number , con_number , fl.lic_currency ,lsl.lsl_number, last_day('''
                             || i_period_date
                             || ''')    ),2)  paid
                    ,  CASE WHEN '''
                             || i_evaluate_val
                    ...
                    *
                    ...
                    PKG_FIN_MNET_OUTSTAND_COMENT.PRC_FIN_MNET_OUSNCOM_PAID(fl.lic_number , con_number , fl.lic_currency ,lsl.lsl_number, last_day('''
                                || i_period_date
                                || ''')    )
                    end,2) <> 0) OR ROUND(PKG_FIN_MNET_OUTSTAND_COMENT.PRC_FIN_MNET_OUSNCOM_PAID(fl.lic_number , con_number , fl.lic_currency ,lsl.lsl_number, last_day('''
                                || i_period_date
   ...
   BEGIN
      ...
      ELSE
          BEGIN
              ...
              THEN
                 OPEN o_cursor FOR
                    SELECT channel_company com_name, 
					       lic_currency,
...
**********************************************************************************************************************************************/
   ...
   BEGIN
				...
				WHERE LIS_PER_YEAR||LPAD(LIS_PER_MONTH,2,0) = TO_CHAR(i_period_date,'RRRRMM')
				AND (lis_lic_status = 'C' OR lis_lic_start > LAST_DAY(i_period_date))
				UNION
				SELECT lsh_lic_number
				FROM(
				SELECT lsh_lic_number,lsh_lic_status,
				DENSE_RANK() OVER
						(PARTITION BY LSH_LIC_NUMBER,LSH_STATUS_YYYYMM ORDER BY LSH_STATUS_YYYYMM DESC) 
						RN
				FROM X_LIC_STATUS_HISTORY
				WHERE LSH_STATUS_YYYYMM <= TO_NUMBER(TO_CHAR(i_period_date,'RRRRMM'))
				)WHERE RN = 1
...
AND    fct.con_short_name LIKE '''|| l_con_short_name || '''
	AND (NOT EXISTS ( 
	                 SELECT  lis_lic_number,NVL(SUM(LIS_CON_FC_EMU),0)
										 FROM    X_MV_OUT_COMM_EX
										WHERE   lis_lic_number = fl.lic_number
										  and   LIS_YYYYMM_NUM <= '|| l_yyyymm_num|| '
										GROUP 
...
AND (
								...
								OR
								( lic_acct_date < ''' || i_period_date || '''
									 AND  EXISTS ( SELECT 1 
																	 FROM x_tbl_gtt_content_filter flsl 
																	WHERE fl.lic_number = flsl.FL_GEN_REFNO
																)
								 )
		 )';
      ...
      l_qry_string := l_qry_string || ' ORDER BY fc.com_name,    
																								 lic_currency,    
																								 lic_type,    
																								 lee_short_name,    
																								 lic_budget_code,    
			...
			--INSERT INTO DEBUG_DATA ( DEBUGID,SP_NAME) VALUES (9999999,to_char(l_qry_string) );
			--COMMIT;
      ...
      L_QUERY_STRING := L_QUERY_STRING ||
                                ...
                                (  CASE
                                      ...
                                      THEN PKG_FIN_MNET_OUTSTAND_COMENT.x_fin_con_forcast_sum
                                                               ...
                                                               ','
                                                                ||l_year||
                                                               ')
                                      ELSE PKG_FIN_MNET_OUTSTAND_COMENT.prc_fin_mnet_ousncom_liab_new
                                                     (fl.lic_number,
                                                      con_number,
                                                      fl.lic_currency,
                                                      LAST_DAY ('''
                                    ...
                                    END
                             + PKG_FIN_MNET_OUTSTAND_COMENT.prc_fin_mnet_ousncom_paid_new
                                                      (fl.lic_number,
                                                       con_number,
                                                       fl.lic_currency,
                                                       lsl.lsl_number,
         ...
         L_QUERY_STRING := L_QUERY_STRING ||'
            ...
            IF i_include_zeros = 'N'
						...
						L_QUERY_STRING := L_QUERY_STRING || ' ORDER BY fc.com_name,
																													 fl.lic_currency,
																													 fl.lic_type,
																													 fle.lee_short_name,
																													 fl.lic_budget_code,
			...
			--INSERT INTO DEBUG_DATA ( DEBUGID,SP_PARMS) VALUES ((select nvl(max(debugid),0)+1 from DEBUG_DATA),to_char(L_QUERY_STRING) );
			--COMMIT;
			--Raise_application_error (-20000,' From date is incorrect - '|| to_char(i_from_date,'dd-mon-rrrr'));
        OPEN O_CURSOR FOR L_QUERY_STRING;
   EXCEPTION
      WHEN OTHERS
      THEN
      ...
      END IF;
         ...
         OPEN o_cursor FOR
            SELECT   com_name, com_number com_number1, lic_type,
                     ...
                     ROUND
                        (SUM
                            (PKG_FIN_MNET_OUTSTAND_COMENT.prc_fin_ousncom_paid_sum_new
                                                      (lic_number,
                                                       lic_currency,
                                                       lsl_number,
                                                       LAST_DAY (i_period_date)
                     ...
                     ROUND
                        (SUM
                            (  PKG_FIN_MNET_OUTSTAND_COMENT.prc_fin_ousncom_liab_sum_new
                                                     (lic_number,
                                                      lic_currency,
                                                      LAST_DAY (i_period_date),
                                                      lsl_lee_price,
      ...
      END IF;
        OPEN o_cursor FOR
            SELECT channel_company com_name, lic_currency,
                   ...
                   ROUND (exchange_rate, 5) exchange_rate,   -- 02-Nov-2016 [Zeshan Khan] [Finance Dev Phase I] Spot rates upto 5 decimals.
                   ROUND (loc_liab, 2) loc_liab
              FROM (SELECT com_name channel_company, 
													 lic_currency,
                           ter_cur_code, lic_type, lee_short_name,
                           lic_budget_code, supplier, con_short_name,
                           lic_number, 
...
FUNCTION prc_fin_ousncom_liab_sum_new (
   ...
   BEGIN
      ...
      RETURN NVL (ROUND (l_liab, 4), 0);
   END prc_fin_ousncom_liab_sum_new;
...
BEGIN
      ...
      RETURN NVL (ROUND (l_paid, 4), 0);
END prc_fin_ousncom_paid_sum_new;
